...
import { book_body_epub } from '../../BookGenerator/BookGenerator';
...
interface IProps {
    ...
    body: book_body_epub[];
    onFileChange: (value: book_body_epub[]) => void;
}
...
class EpubGeneratorComponent extends BaseComponent<IProps, IState> {
    ...
    componentDidMount() {
        this.setState({
            epub: (this.props.body.length && this.props.body[0].epub && this.props.body[0].epub !== '') ? this.props.body[0].epub : [],
            name: (this.props.body.length && this.props.body[0].name && typeof this.props.body[0].name === 'string') ? this.props.body[0].name : '',
        });
        this.epub = (this.props.body.length && this.props.body[0].epub && this.props.body[0].epub !== '') ? this.props.body[0].epub : [];
    ...
    componentWillReceiveProps(nextProps: IProps) {
        ...
        this.setState({
            epub: (nextProps.body.length && nextProps.body[0].epub && nextProps.body[0].epub !== '') ? nextProps.body[0].epub : [],
            name: (nextProps.body.length && nextProps.body[0].name && typeof nextProps.body[0].name === 'string') ? nextProps.body[0].name : '',
        });
    }

    passBodyObjToProps() {
        let newBody: book_body_epub[] = [
            {
                ...
                type: (this.props.body.length && this.props.body[0].type) ? this.props.body[0].type : 'epub',
            }
        ];
    ...
    onDropPdf(file: any[]) {
        ...
        if (this.state.epub && this.state.epub!.length) {
            this.removePreviousPdfNotify();
            return;
        }
        this.epub = file;
        this.name = this.epub[0].name ? this.epub[0].name : '';
        this.setState({
            ...this.state,
            epub: this.epub,
            name: this.name,
    ...
    returner_pdf_render() {
        if (typeof this.state.epub === 'string') {
                                                ...
                                                <div className="img-item m-2">
                                                    ...
                                                    }
                                                    {
                                                        this.state.epub.length === 0
                                                            ...
                                                            undefined
                                                            :
                                                            <span className="mx-2 text-dark">{this.state.name ? this.state.name : ""} {(this.epub[0].size && typeof this.epub[0].size === "number") ? '- ' + parseFloat((this.epub[0].size / 1024).toFixed(2)) + ' KB' : 'Unknown size'}</span>
                                                    }
...
export const EpubGenerator = connect(
    state2props,
    dispatch2props
)(EpubGeneratorComponent);